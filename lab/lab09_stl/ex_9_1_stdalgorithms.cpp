...
void print(const Container & c){
    ...
    // <<>> (1 line)
    copy(c.begin(), c.end(), std::ostream_iterator<typename Container::value_type>(std::cout, ", "));
    std::cout << std::endl;
}
...
// <<>> (several lines)
    ...
    return equal(word1.begin(), word1.end(), word2.begin());
...
int main(){
    ...
    // <<>> (1 line)
    copy_if(v.begin(), v.end(), std::back_inserter(odd), [](int x){ return x % 2 != 0; });
    print(odd);

    //6. Each number x in v replace with x*x-1 (Hint std::transform)
    // <<>> (1 line)
    transform(v.begin(), v.end(), v.begin(), [](int x){ return x*x - 1; });
    print(v);
    ...
    // <<>> (2 lines)
    sort(v.begin(), v.end());
    set_intersection(odd.begin(), odd.end(), v.begin(), v.end(), std::back_inserter(intersect));
    print(intersect);
