...
int main()
{
    ...
    std::sort(v.begin(), v.end(), [&center](int a, int b)
              { return abs(center - a) < abs(center - b); }); // 1 - DONE
    print(v, "center 50:");
    ...
    {
        int asign = (a < 0) ? (-1) : (a == 0 ? 0 : 1);
        int bsign = (b < 0) ? (-1) : (b == 0 ? 0 : 1);
        if (a * b == 0)
            return (a > b);
        if (asign * bsign < 0)
            return a > b;
    ...
    auto l1_norm_helper = [](auto a, auto b)
    { return abs(a) + abs(b); };

    auto l1_norm = [&l1_norm_helper](auto v)
    {
        return std::accumulate(v.begin(), v.end(), 0.0, l1_norm_helper); // [ 5 ]
    };
